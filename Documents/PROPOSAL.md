# Product Proposal — Midnight-Gate

**Program:** New Moon to Full: Monthly Moonshots on Midnight
**Project:** Midnight-Gate — Private Access & Credential Verification
...
Midnight-Gate is designed to make that verification private.

## Product

**Midnight-Gate** is a reusable private access and credential verification layer built on Midnight.
...
Midnight-Gate is intended to become infrastructure that other Midnight developers can reuse instead of rebuilding private access logic for every application.

The MVP will provide a simple **Midnight-Gate SDK/API** for developers to:
...
The SDK will handle the Midnight-Gate verification flow, including interacting with the credential/access contract and returning the verification result to the integrating application.

This allows another dApp to use Midnight-Gate as a private access layer without implementing its own credential verification system from scratch.

## Sample dApp — Midnight-Gate Developer Portal

To demonstrate that Midnight-Gate works as reusable infrastructure, I will build a small Midnight dApp that integrates the Midnight-Gate SDK.
...
**Issuer creates credential → User receives credential → User opens protected dApp → dApp requests Midnight-Gate verification → Zero Knowledge proof verifies eligibility → Midnight-Gate returns verification result → Protected feature unlocks**
...
This sample dApp will demonstrate the full integration rather than showing Midnight-Gate only as an isolated proof-of-concept.

## Why Midnight

Midnight-Gate needs users to prove facts about private credentials without exposing the credentials themselves.
...
With Midnight-Gate on Midnight, the application can verify:
...
That privacy boundary is the main reason Midnight-Gate is being built on Midnight.
...
6. Build the Midnight-Gate SDK/API for external dApp integration.
7. Build a sample Midnight Developer Portal protected using the Midnight-Gate SDK.
...
Midnight-Gate should not be only a single credential-gated application.
